...
def test_rect_vs_rect_doesnt_collide():
    assert not rect_vs_rect(
        AABB(0, Vec2(-10, -10), 10, 10), AABB(1, Vec2(10, 10), 10, 10)
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            AABB(0, Vec2(-1, 0), 10, 10),
            AABB(1, Vec2(0, 0), 10, 10),
            Vec2(-9, 0),
        ),  # Collides on left
        (
            AABB(0, Vec2(1, 0), 10, 10),
...
def test_collision_process(ecs_world):
    ...
    colliding1 = ecs.create_entity(
        Position(Vec2(0, 0)), BoxCollider(height=10, width=10)
    )
    colliding2 = ecs.create_entity(
        Position(Vec2(0, 0)), BoxCollider(height=10, width=10)
    )
    not_colliding = ecs.create_entity(
        Position(Vec2(20, 20)), BoxCollider(height=10, width=10)
    )
